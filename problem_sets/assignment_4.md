...
``` r
ggplot(data = economist_data, aes(x = CPI, y = HDI)) +
...
``` r
ggplot(data = economist_data, aes(x = HDI)) +
  geom_density(mapping = aes(fill = Region), alpha = .5)
```
...
ggplot(data = economist_data, aes(x = Region, y = HDI)) +
  geom_boxplot(mapping = aes(color = Region, fill = Region), alpha = .5, outlier.shape = NA) +
...
``` r
# create a bar graph showing the number of countries in each region in economist_data
ggplot(data = economist_data, aes(x = Region)) +
  geom_bar()
```
...
Answer: *I think 1.6 is the best overview. 1.8 is also has some pretty
...
``` r
select(Theoph, contains("t", ignore.case = FALSE))|> 
  head(6) |> 
  kable()
```
